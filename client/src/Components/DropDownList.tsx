...
function DropDownList({
      ...
      setFilteredOptions(
        options.filter((option) =>
          Array.isArray(option)
            ? false
            : option.toLowerCase().search(value.toLowerCase()) !== -1
              ? true
              : false
        )
      );
  ...
  return (
    ...
    >
      <div
        className="select-btn hover:shadow-xl flex justify-between h-16 py-0 px-5 border-2 rounded-md border-grey "
        onClick={openOptions}
      >
        ...
        <div className="self-center flex gap-3">
          ...
          >
            <div
              ref={closeIconRef}
              className={` ${rotate} duration-300 close-icon`}
            >
              <CgClose />
            </div>
          </CSSTransition>
          <div className={` ${rotate} duration-300`}>
            <CgChevronDown />
          </div>
        </div>
      </div>
      ...
      >
        <div
          ref={optionsRef}
          className="content mt-4 border-2 border-grey rounded-md p-5 relative shadow-xl"
        >
          <div className="search flex items-baseline relative">
            <div className="absolute top-5 left-1 text-[#999]">
              <CgSearch />
            </div>
            <input
          ...
          >
            {filteredOptions.map((option, index) => {
              const optionStatus = Array.isArray(option)
                ? "disabled-option"
                : "active-option";
              ...
              return (
                <li
                  ...
                  value={value}
                  className={`
                ${optionStatus === "active-option"
                      ? "active-option hover:bg-blue hover:text-white"
                      : "disabled-option text-[#999]"
                    }
                hover:rounded-md
                                cursor-pointer
 h-16
